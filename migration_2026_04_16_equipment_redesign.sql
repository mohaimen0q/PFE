-- =========================================================
-- Migration: 2026-04-16 — Equipment module redesign
-- Adds asset_code column, updates statuses, reseeds data
-- =========================================================

-- 1. Add asset_code column if it doesn't exist
ALTER TABLE equipment
    ADD COLUMN IF NOT EXISTS asset_code VARCHAR(50) UNIQUE;

-- 2. Add label to meter_thresholds if it doesn't exist
ALTER TABLE meter_thresholds
    ADD COLUMN IF NOT EXISTS label VARCHAR(100);

-- 2. Extend status enum to include new values (PostgreSQL)
DO $$
BEGIN
    ALTER TYPE equipment_status ADD VALUE IF NOT EXISTS 'OUT_OF_SERVICE';
    ALTER TYPE equipment_status ADD VALUE IF NOT EXISTS 'REFORMED';
EXCEPTION WHEN others THEN
    -- Not a PostgreSQL enum type; handled by JPA
    NULL;
END $$;

-- 3. Set default status to OPERATIONAL
ALTER TABLE equipment
    ALTER COLUMN status SET DEFAULT 'OPERATIONAL';

-- 4. Remove old seed data
...
SELECT
    ...
    d.department_id, t.classification, t.criticality, t.meter_unit, t.start_meter_value,
...
-- 6. Create meters for all newly inserted equipment
...
-- 7. Seed thresholds
INSERT INTO meter_thresholds (meter_id, threshold_value)
SELECT m.meter_id, t.threshold_value
FROM meters m
...
JOIN (VALUES
    ...
    ('EQ-OR-005',  1000),
    ('EQ-NEO-001', 1200)
) AS t(asset_code, threshold_value) ON t.asset_code = e.asset_code
